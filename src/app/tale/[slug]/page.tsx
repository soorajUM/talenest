...
export default async function Tale({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
        <div>
            ...
            < main className="p-2 grid gap-2 max-w-4xl m-auto" >
                <div key={story.title} className="shadow p-4" >
                    <div className="text-xl text-slate-900 pb-4 font-bold" > {story.title} </div>
                    < div className=" text-gray-600 grid gap-4 text-lg" > {
                        story.content.map((text, i) =>
                            <div key={i}>
                                {text}
                            </div>
                        )
